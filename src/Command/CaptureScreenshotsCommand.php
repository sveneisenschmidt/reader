...
class CaptureScreenshotsCommand extends Command
{
    ...
    private function addFeed(
        ...
        if (count($detailsElements) > 0) {
            ...
            // Update name
            $nameInput = $lastDetails->findElement(
            ...
            if ($folder !== null) {
                $folderInput = $lastDetails->findElement(
                    WebDriverBy::cssSelector(
            ...
            // Save immediately
            $saveButton = $lastDetails->findElements(
                WebDriverBy::cssSelector('.form-actions button[type="submit"]'),
            );
