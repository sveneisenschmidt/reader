...
class MigrateDataCommand extends Command
{
    ...
    protected function execute(
        ...
        $subscriptionsDb = $this->resolvePath($input->getOption('subscriptions-db'));
        ...
        $this->migrateDatabase($usersDb, 'old_users', [
            'user',
            'user_preference',
            'read_status',
            'seen_status',
        ...
        $this->migrateDatabase($subscriptionsDb, 'old_subs', [
            'subscription',
        ], $io);

        // Migrate content database
        $io->section('Migrating content database');
        $this->migrateDatabase($contentDb, 'old_content', [
            'feed_item',
        ], $io);

        // Migrate messages database
        $io->section('Migrating messages database');
        $this->migrateDatabase($messagesDb, 'old_msgs', [
            'processed_message',
        ], $io);
    ...
    private function migrateDatabase(
        ...
        foreach ($tables as $table) {
            ...
            $this->connection->executeStatement(
                sprintf('INSERT INTO %s SELECT * FROM %s.%s', $table, $alias, $table),
            );
