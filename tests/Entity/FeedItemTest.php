...
class FeedItemTest extends TestCase
{
    ...
    #[Test]
    public function updateFetchedAtUpdatesTimestamp(): void
    {
        $item = $this->createFeedItem();
        $originalFetchedAt = $item->getFetchedAt();

        usleep(1000); // Wait 1ms
        $result = $item->updateFetchedAt();

        $this->assertGreaterThan($originalFetchedAt, $item->getFetchedAt());
        $this->assertSame($item, $result);
    }
